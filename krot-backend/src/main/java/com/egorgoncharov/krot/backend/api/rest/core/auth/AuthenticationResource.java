package com.egorgoncharov.krot.backend.api.rest.core.auth;

import com.egorgoncharov.krot.backend.api.rest.core.auth.request.AuthenticationHandshakeRequest;
import com.egorgoncharov.krot.backend.api.rest.core.auth.response.AuthenticationCredentials;
...
public class AuthenticationResource {
    private final AuthenticationManagerImpl authenticationService;

    @Inject
    public AuthenticationResource(AuthenticationManagerImpl authenticationService) {
    ...
    public Uni<RestResponse<ApiResponse<Void>>> authDisconnect(@Valid RequestWithMetadata body) {
        return authenticationService.logout(Types.toUUID(body.getMetadata().getSessionId())).map(result -> result.voidCast().toRestResponse());
    }
}
