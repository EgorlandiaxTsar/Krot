package com.egorgoncharov.krot.backend.api.rest.core.user.response;
...
public class User implements EntityReflection<UserEntity> {
    ...
    public UserEntity to() {
        return UserEntity.builder().id(Types.toUUID(id)).username(username).role(RoleEntity.builder().id(Types.toUUID(roleId)).build()).active(active).createdAt(OffsetDateTime.ofInstant(Instant.ofEpochMilli(createdAt), ZoneId.of("UTC"))).build();
    }
