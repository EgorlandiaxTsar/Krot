...
public class HttpEncryptionFilter {
    ...
    private final TCPTransportManagerImpl tcpTransportService;
    private final ObjectMapper objectMapper;

    @Inject
    public HttpEncryptionFilter(STPConfig stpConfig, TCPTransportManagerImpl tcpTransportService, ObjectMapper objectMapper) {
    ...
    public void encrypt(ContainerResponseContext context, RoutingContext vertxContext) {
        ...
        context.setEntity(encryptedResponse.body());
        context.getHeaders().add(TAG_HEADER, encryptedResponse.tag());
        context.getHeaders().add(NONCE_HEADER, encryptedResponse.nonce());
    }
}
