...
public class DeviceContextLoader extends AbstractCrudContextLoader<DeviceEntity, UUID, Void, DeviceQuery> {
    ...
    @Override
    public Uni<Result<? extends QueryContext<DeviceEntity, UUID, Void>>> queryContext(DeviceQuery query, Void subcontext) {
        ...
        return executeQuery(queryBuilder.toString(), parameters, query).map(result -> {
            ...
            QueryContext<DeviceEntity, UUID, Void> context = result.getResult().get();
    ...
    @Override
    public Uni<Result<? extends CreationContext<DeviceEntity, UUID, Void>>> creationContext(DeviceEntity entity, Void subcontext) {
        return super.creationContext(entity, subcontext).chain(contextResult -> {
            ...
            CreationContext<DeviceEntity, UUID, Void> context = contextResult.getResult().get();
    ...
    @Override
    public Uni<Result<? extends UpdateContext<DeviceEntity, UUID, Void>>> updateContext(DeviceEntity entity, Void subcontext) {
        return super.updateContext(entity, subcontext).chain(contextResult -> {
            if (contextResult.getCode() != 200 || contextResult.getResult().isEmpty()) return contextResult.toUni();
            UpdateContext<DeviceEntity, UUID, Void> context = contextResult.getResult().get();
    ...
    public Uni<Result<? extends UpdateContext<DeviceEntity, UUID, Void>>> collaboratorUpsertContext(DeviceEntity entity) {
        return collaboratorManageContext(entity).chain(contextResult -> {
            ...
            return userRepository.findById(targetCollaborator.getCollaborator().getId())
                    .map(Objects::nonNull)
                    .chain(exists -> exists ? contextResult.toUni() : Result.<UpdateContext<DeviceEntity, UUID, Void>>notFound().toUni());
        });
    }

    public Uni<Result<? extends UpdateContext<DeviceEntity, UUID, Void>>> collaboratorDeleteContext(DeviceEntity entity) {
        return collaboratorManageContext(entity);
    }

    public Uni<Result<? extends UpdateContext<DeviceEntity, UUID, Void>>> ownershipTransferContext(DeviceEntity entity) {
    ...
    protected Uni<Result<? extends UpdateContext<DeviceEntity, UUID, Void>>> collaboratorManageContext(DeviceEntity entity) {
