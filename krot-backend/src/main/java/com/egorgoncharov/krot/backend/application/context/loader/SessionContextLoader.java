...
public class SessionContextLoader extends AbstractCrudContextLoader<HistoricalSessionEntity, UUID, Void, SessionQuery> {
    ...
    @Override
    public Uni<Result<? extends QueryContext<HistoricalSessionEntity, UUID, Void>>> queryContext(SessionQuery query, Void subcontext) {
    ...
    @Override
    public Uni<Result<? extends CreationContext<HistoricalSessionEntity, UUID, Void>>> creationContext(HistoricalSessionEntity entity, Void subcontext) {
        return super.creationContext(entity, subcontext).chain(contextResult -> {
            ...
            CreationContext<HistoricalSessionEntity, UUID, Void> context = contextResult.getResult().get();
            ...
            return entityUni.chain(owner -> {
                if (owner == null) return Result.<CreationContext<HistoricalSessionEntity, UUID, Void>>notFound().toUni();
                String entityPassword = isUser ? ((UserEntity) (owner)).getPassword() : ((DeviceEntity) (owner)).getPassword();
                if (!password.equals(entityPassword)) return Result.<CreationContext<HistoricalSessionEntity, UUID, Void>>forbidden().toUni();
                if (isUser) {
    ...
    @Override
    public Uni<Result<? extends UpdateContext<HistoricalSessionEntity, UUID, Void>>> updateContext(HistoricalSessionEntity entity, Void subcontext) {
        return null;
    }

    @Override
    public Uni<Result<? extends DeleteContext<HistoricalSessionEntity, UUID, Void>>> deleteContext(List<HistoricalSessionEntity> entities, Void subcontext) {
        return null;
    }
}
