...
public class AuthenticationResource {
    ...
    public Uni<RestResponse<ApiResponse<AuthenticationCredentials>>> authHandshake(RoutingContext context, @Valid AuthenticationHandshakeRequest handshakeCredentials) {
        ...
        return authenticationService.create(SessionEntity.builder().userOwner(handshakeSession.getTarget() == PrincipalType.USER ? UserEntity.builder().id(TypesHelper.toUUID(handshakeCredentials.getId())).password(handshakeCredentials.getPassword()).build() : null).deviceOwner(handshakeSession.getTarget() == PrincipalType.DEVICE ? DeviceEntity.builder().id(TypesHelper.toUUID(handshakeCredentials.getId())).password(handshakeCredentials.getPassword()).build() : null).handshakeKey(handshakeSession.getKey()).build()).chain(result -> {
            ...
            return new ApiResponse<>(result.toApiMetadata(), new AuthenticationCredentials(session.getSrcKey().toString(), session.getId().toString(), Base64.getEncoder().encodeToString(session.getEncryptionKey()), session.getValidUntil().toInstant().toEpochMilli()), null).toUniRestResponse();
        });
    }
}
