package com.egorgoncharov.krot.backend.database.redis.repository;

import com.egorgoncharov.krot.backend.database.redis.RedisCrudRepository;
...
@ApplicationScoped
public class SessionRepository implements RedisCrudRepository<SessionEntity, UUID> {
    ...
    public Uni<SessionEntity> save(SessionEntity entity) {
        return RedisCrudRepository.super.save(entity).flatMap(savedEntity -> {
    ...
    public Uni<SessionEntity> save(SessionEntity entity, Duration ttl) {
        return RedisCrudRepository.super.save(entity, ttl).flatMap(savedEntity -> {
            if (savedEntity.getSessionReference() == null) return Uni.createFrom().item(savedEntity);
            String indexKey = buildReferenceIndexKey(savedEntity.getSessionReference());
