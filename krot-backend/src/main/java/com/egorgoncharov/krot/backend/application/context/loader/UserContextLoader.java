...
public class UserContextLoader extends AbstractCrudContextLoader<UserEntity, UUID, Void, UserQuery> {
    ...
    @Override
    public Uni<Result<? extends QueryContext<UserEntity, UUID, Void>>> queryContext(UserQuery query, Void subcontext) {
    ...
    @Override
    public Uni<Result<? extends CreationContext<UserEntity, UUID, Void>>> creationContext(UserEntity entity, Void subcontext) {
        return super.creationContext(entity, subcontext).chain(contextResult -> {
            ...
            CreationContext<UserEntity, UUID, Void> context = contextResult.getResult().get();
            ...
            return roleRepository.findById(context.getEntity().getRole().getId())
                    .map(Objects::nonNull)
                    .chain(exists -> exists ? contextResult.toUni() : Result.<CreationContext<UserEntity, UUID, Void>>notFound().toUni());
        });
    }

    @Override
    public Uni<Result<? extends UpdateContext<UserEntity, UUID, Void>>> updateContext(UserEntity entity, Void subcontext) {
        return super.updateContext(entity, subcontext).chain(contextResult -> {
            if (contextResult.getCode() != 200 || contextResult.getResult().isEmpty()) return contextResult.toUni();
            UpdateContext<UserEntity, UUID, Void> context = contextResult.getResult().get();
            ...
            return roleExistsUni.chain(exists -> exists ? contextResult.toUni() : Result.<UpdateContext<UserEntity, UUID, Void>>notFound().toUni());
        });
    }

    public Uni<Result<? extends UpdateContext<UserEntity, UUID, Void>>> updatePasswordContext(UserEntity entity, String oldPassword) {
