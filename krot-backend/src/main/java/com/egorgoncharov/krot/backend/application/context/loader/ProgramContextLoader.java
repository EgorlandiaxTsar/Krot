...
public class ProgramContextLoader extends AbstractCrudContextLoader<ProgramEntity, UUID, Void, ProgramQuery> {
    ...
    @Override
    public Uni<Result<? extends QueryContext<ProgramEntity, UUID, Void>>> queryContext(ProgramQuery query, Void subcontext) {
    ...
    @Override
    public Uni<Result<? extends CreationContext<ProgramEntity, UUID, Void>>> creationContext(ProgramEntity entity, Void subcontext) {
        return super.creationContext(entity, subcontext).chain(contextResult -> {
            ...
            CreationContext<ProgramEntity, UUID, Void> context = contextResult.getResult().get();
    ...
    @Override
    public Uni<Result<? extends UpdateContext<ProgramEntity, UUID, Void>>> updateContext(ProgramEntity entity, Void subcontext) {
        return super.updateContext(entity, subcontext).chain(contextResult -> {
            if (contextResult.getCode() != 200 || contextResult.getResult().isEmpty()) return contextResult.toUni();
            UpdateContext<ProgramEntity, UUID, Void> context = contextResult.getResult().get();
    ...
    public Uni<Result<? extends UpdateContext<ProgramEntity, UUID, Void>>> collaboratorUpsertContext(ProgramEntity entity) {
        return collaboratorManageContext(entity).chain(contextResult -> {
            ...
            return userRepository.findById(targetCollaborator.getCollaborator().getId())
                    .map(Objects::nonNull)
                    .chain(exists -> exists ? contextResult.toUni() : Result.<UpdateContext<ProgramEntity, UUID, Void>>notFound().toUni());
        });
    }

    public Uni<Result<? extends UpdateContext<ProgramEntity, UUID, Void>>> collaboratorDeleteContext(ProgramEntity entity) {
        return collaboratorManageContext(entity);
    }

    public Uni<Result<? extends UpdateContext<ProgramEntity, UUID, Void>>> ownershipTransferContext(ProgramEntity entity) {
    ...
    protected Uni<Result<? extends UpdateContext<ProgramEntity, UUID, Void>>> collaboratorManageContext(ProgramEntity entity) {
