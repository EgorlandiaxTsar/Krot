package com.egorgoncharov.krot.backend.security.filters.http;
...
public class HttpDecryptionFilter {
    ...
    @Inject
    public HttpDecryptionFilter(TCPTransportManagerImpl transportManager) {
    ...
    public Uni<Response> decrypt(ContainerRequestContext context, RoutingContext vertxContext) {
        ...
        if (path.equals("/api/auth/handshake")) {
            Result<HandshakeSession> result = transportManager.establishHandshakeRequest(Buffer.buffer(originalBody), HandshakeHeaders.fromRoutingContext(vertxContext));
            ...
            return transportManager.establishRequest(Buffer.buffer(originalBody), RequestHeaders.fromRoutingContext(vertxContext)).chain(result -> {
                if (result.getCode() == 200 && result.getResult().isPresent()) {
